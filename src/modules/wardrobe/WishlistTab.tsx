...
export default function WishlistTab() {
  const { data: items = [], isLoading } = useWardrobeItems('wishlist')
  const upsert = useUpsertWardrobeItem()
  const del = useDeleteWardrobeItem()
  const markOwned = useMarkAsOwned()
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {isLoading ? (
        ...
        <EmptyState
          message={items.length === 0 ? 'Your wishlist is empty.' : 'No items match those filters.'}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-4 gap-y-6">
          {filtered.map((item) => (
            <ItemCard
              ...
              onMarkOwned={(id) => markOwned.mutate(id)}
            />
          ))}
        </div>
      )}
